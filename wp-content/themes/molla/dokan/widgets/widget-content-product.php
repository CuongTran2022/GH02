...
if ( class_exists( 'WooCommerce' ) ) {
	$show_info = wc_get_loop_prop( 'product_show_op', array() );
}

?>

<?php if ( $r->have_posts() ) : ?>
	<ul class="dokan-bestselling-product-widget products-widget-loop">
	<?php
	while ( $r->have_posts() ) :
		$r->the_post();
		?>
		...
		<li class="product-widget product product-loop product-wrap">
			<a class="product-media" href="<?php echo esc_url( get_permalink( dokan_get_prop( $product, 'id' ) ) ); ?>" title="<?php echo esc_attr( $product->get_title() ); ?>">
				<?php echo molla_strip_script_tags( $product->get_image() ); ?>
			</a>
			<div class="product-details">
				...
				<?php else : ?>
					<?php
					if ( ! empty( $show_rating ) ) {
						echo molla_filter_output( $product->get_rating_html() ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped 
					}
					?>
				<?php endif ?>
				</div>
				...
				if ( in_array( 'price', $show_info ) ) {
					echo '<span class="price">' . molla_filter_output( $product->get_price_html() ) . '</span>'; // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped 
				}
				?>
			</div>
		</li>
