...
class Updater {
	 ...
	 * @var array
	 */
	private $allowedTaxonomies = array();
	...
	public function setAllowedTaxonomies() {
		$this->allowedTaxonomies = array(
			'product_cat',
			'product_tag'
		);

		if ( DGWT_WCAS()->brands->hasBrands() && DGWT_WCAS()->settings->getOption( 'search_in_brands' ) === 'on' ) {
			$this->allowedTaxonomies[] = DGWT_WCAS()->brands->getBrandTaxonomy();
		}
	...
	private function maybeUpdateProducts( $postsIDs, $taxonomy ) {
		...
		if (
			( $taxonomy === 'product_cat' && DGWT_WCAS()->settings->getOption( 'search_in_product_categories' ) === 'on' ) ||
			( $taxonomy === 'product_tag' && DGWT_WCAS()->settings->getOption( 'search_in_product_tags' ) === 'on' ) ||
			( DGWT_WCAS()->brands->hasBrands() && $taxonomy === DGWT_WCAS()->brands->getBrandTaxonomy() && DGWT_WCAS()->settings->getOption( 'search_in_brands' ) === 'on' ) ||
