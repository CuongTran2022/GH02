...
class AjaxQuery extends MainQuery {
	...
	public static function sendEmptyResponse( $engine = 'pro' ) {
		...
		$output['v']      = Config::ENGINE_VERSION;
	...
	public function sendResults( $exit = true ) {
		...
		foreach ( $this->foundTax as $suggestion ) {
			...
			$key = ! empty( $suggestion['taxonomy'] ) ? $suggestion['taxonomy'] : '';
	...
	private function searchResultsGroups() {
		...
		// Categories
		if ( $this->getOption( 'show_matching_categories', 'string', 'off' ) === 'on' ) {
		...
		// Tags
		if ( $this->getOption( 'show_matching_tags', 'string', 'off' ) === 'on' ) {
		...
		// Brands
		if ( $this->getOption( 'show_matching_brands', 'string', 'off' ) === 'on' ) {
			$groups['brand'] = array(
				'limit' => 3,
				'order' => 5
			);
