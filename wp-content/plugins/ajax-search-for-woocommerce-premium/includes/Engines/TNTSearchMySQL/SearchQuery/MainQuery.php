...
abstract class MainQuery {
	 ...
	 * @return string
	 */
	protected function getPhrase( $mode = 'readable' ) {
		$phrase = trim( $this->s );
	...
	public function searchTaxonomy() {
		...
		if ( $taxQuery->isEnabled() ) {

			if ( ! empty( $this->lang ) ) {
				$taxQuery->setLang( $this->lang );
			}

			$this->foundTax = $taxQuery->search( $this->getPhrase() );
	...
	private function orderByWeight() {
		...
		foreach ( $this->foundProducts as $product ) {
			...
			$this->foundProducts[ $i ]->score = apply_filters('dgwt/wcas/tnt/product/score', $score, $product->post_id, $product );
