...
<?php
  ...
  public function enqueue_scripts()
  {
    ...
    wp_register_script(
      ...
      PWB_PLUGIN_URL . '/assets/js/functions-frontend.js',
      array('jquery'),
      PWB_PLUGIN_VERSION,
      true
    );
  ...
  public function admin_enqueue_scripts($hook)
  {
    wp_enqueue_style(
      'pwb-select2-css',
      PWB_PLUGIN_URL . '/assets/css/select2.min.css',
      array(),
      '4.0.3',
      'all'
    );
    ...
    wp_register_script(
      'pwb-select2-script',
      PWB_PLUGIN_URL . '/assets/js/select2.min.js',
      array('jquery'),
      '4.0.3',
      true
    );
    wp_register_script(
      'pwb-custom-brand-address',
      PWB_PLUGIN_URL . '/assets/js/custom-brand-with-address.js',
      array('jquery'),
      '1.0.0',
      true
    );
    ...
    wp_enqueue_script('pwb-select2-script');
    wp_enqueue_script('pwb-custom-brand-address');
  }
  ...
  public static function get_brands($hide_empty = false, $order_by = 'name', $order = 'ASC', $only_featured = false, $pwb_term = false, $only_parents = false, $addressSearch)
  {
    $result = array();
    ...
    if(!empty($addressSearch)){
      ...
      foreach ($addressSearch as $key => $value) {
        array_push($query_meta, array('key' => $key, 'value' => $value));
      }
     $brands_args['meta_query'] = $query_meta;
    }
  ...
  public static function get_districts($provinceId)
  {
  ...
  public static function get_wards($districtId)
  {
