    ...
    $(document).ready(function () {
        ...
        if (jqueryProvince.length > 0) {
            $('#pwd_brand_province').select2({ id: '', placeholder: "Chọn Tỉnh/Thành phố", }).on('select2:select', function (e) {
                handleLoading("#dt_wrap", true);
                var provinceId = e.params.data.id
                $.ajax({
                    type: "post",
                    dataType: "json",
                    url: pwb_ajax_object.ajax_url,
                    data: {
                        action: "getDistricts",
                    ...
                    success: function (response) {
                        $('#pwd_brand_district').select2({
                            data: response.data
                        });
                        handleLoading("#dt_wrap", false);
                    },
            ...
            $('#pwd_brand_district').select2({ id: '', placeholder: "Chọn Quận/Huyện" }).on('select2:select', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        $('#pwd_brand_ward').select2({
                            data: response.data
                        });
                        handleLoading("#wa_wrap", false);
            ...
            $('#pwd_brand_ward').select2({ id: '', placeholder: "Chọn Phường/Xã" });

            function handleLoading(id, status){
                var idQuery = id + '> span.select2';
                if(status){
                    $(idQuery).addClass("molla-loading")
                }else{
                    $(idQuery).removeClass("molla-loading")
