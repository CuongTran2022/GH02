...
// 		$show_columns['product_tag'] = __( 'Trường', 'woocommerce' ); // ROTICA: show pwb-brand
	...
	protected function render_product_tag_column() {
		...
		// }
		$terms = get_the_terms( $this->object->get_id(), 'pwb-brand' );
		if ( ! $terms ) {
			echo '<span class="na">&ndash;</span>';
		} else {
			$termlist = array();
			foreach ( $terms as $term ) {
				$termlist[] = '<a href="' . esc_url( admin_url( 'edit.php?pwb-brand=' . $term->slug . '&post_type=product' ) ) . ' ">' . esc_html( $term->name ) . '</a>';
			}

			echo apply_filters( 'woocommerce_admin_product_term_list', implode( ', ', $termlist ), 'pwb-brand', $this->object->get_id(), $termlist, $terms ); // WPCS: XSS ok.
		}
	}
